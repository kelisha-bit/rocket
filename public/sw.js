const CACHE_NAME = 'greaterworks-v1.0.0';
...
async function doBackgroundSync() {
  ...
  // For example, sync attendance records, member updates, etc.
}
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: event.data ? event.data.text() : 'New notification from GreaterWorks',
  ...
  event.waitUntil(
    self.registration.showNotification('GreaterWorks Church', options)
  );
});
