...
import { 
  Users, UserPlus, Calendar, Clock, MapPin, Phone, Mail, 
  Activity, TrendingUp, BookOpen, Heart, Target, DollarSign, 
  Sparkles, Plus, Edit2, Trash2, CheckCircle, XCircle, Filter,
...
export default function MenMinistryPage() {
  ...
  // Fetch ministry and members from database
  useEffect(() => {
    const loadMinistryData = async () => {
      try {
        ...
        const ministryData = await fetchMinistryByName('Men Ministry');
        setMinistry(ministryData);
        
        // Fetch ministry members if ministry exists
        if (ministryData?.id) {
          const ministryMembers = await fetchMinistryMembers(ministryData.id);
          setMembers(ministryMembers);
          
          // Load analytics data
          loadAnalyticsData(ministryData.id, ministryData.name);
        } else {
          // Fallback: filter all members by gender
          const fetchedMembers = await fetchMembers();
          const maleMembers = fetchedMembers.filter(member => 
  ...
  const averageAttendanceRate = attendance.length > 0
  ...
  return (
    <AppLayout currentPath="/ministries/men">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage your ministry members, track attendance, and schedule events
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg p-6 shadow-lg">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-sm text-blue-100">Ministry Leader</p>
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-blue-100">{ministry.meeting_time}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-lg shadow-sm border">
          ...
          <div className="p-6">
            ...
            {activeTab === 'contributions' && (
              <ContributionsTable contributions={contributions} loading={loadingAnalytics} />
            )}
            {activeTab === 'events' && renderEventsTab()}
            {activeTab === 'activities' && (
              <ActivityTimeline activities={activities} loading={loadingAnalytics} />
            )}
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Men's Prayer Breakfast"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEventFormData({ ...eventFormData, expected_attendance: parseInt(e.target.value) || 0 })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              placeholder="50"
            />
          </div>
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-600">{attendance.slice(0, 1).reduce((sum, a) => sum + a.present_count, 0)}</p>
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  {member.date_of_birth && (
                    <div className="flex items-center gap-2">
                      ...
                      <span>{new Date().getFullYear() - new Date(member.date_of_birth).getFullYear()} years old</span>
                    </div>
                  )}
                </div>
