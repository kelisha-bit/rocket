  ...
  useEffect(() => {
    const loadMinistryData = async () => {
      try {
        ...
        let ministryData = await fetchMinistryByName('Youth Ministry');
        if (!ministryData) ministryData = await fetchMinistryByName('Youth');
        ...
        if (ministryData?.id) {
          ...
          const youthMembers = fetchedMembers.filter((member) => {
    ...
    <AppLayout currentPath="/ministries/youth">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Youth Ministry Dashboard</h1>
        ...
        {ministry?.leader && (
          <div className="bg-gradient-to-r from-orange-600 to-amber-600 text-white rounded-lg p-6 shadow-lg">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center text-2xl font-bold backdrop-blur-sm">
                {ministry.leader.full_name?.charAt(0) || 'L'}
              </div>
              <div className="flex-1">
                <p className="text-sm text-orange-100">Ministry Leader</p>
              ...
              <div className="text-right">
                <p className="text-sm text-orange-100">Next Meeting</p>
                <p className="text-lg font-semibold">{ministry.meeting_day}</p>
                <p className="text-sm text-orange-100">{ministry.meeting_time}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <MetricCard label="Total Members" value={totalMembers.toString()} icon={<Users className="h-6 w-6 text-orange-600" />} trend={12} />
          <MetricCard label="Active Members" value={activeMembers.length.toString()} icon={<Activity className="h-6 w-6 text-green-600" />} trend={8} />
          <MetricCard label="Avg Attendance" value={`${averageAttendanceRate}%`} icon={<Target className="h-6 w-6 text-blue-600" />} subValue="Last 30 days" />
          <MetricCard label="Contributions" value={`$${totalContributions.toLocaleString()}`} icon={<DollarSign className="h-6 w-6 text-green-600" />} subValue="This month" />
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Youth Encounter Night"
            />
          </div>
          ...
          <div className="flex gap-3 pt-4">
            <button onClick={handleCreateEvent} className="flex-1 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors">
              Create Event
            </button>
            ...
            <div className="space-y-3 text-sm">
              {selectedMember.phone && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`tel:${selectedMember.phone}`} className="text-orange-600 hover:underline">
                    {selectedMember.phone}
                  </a>
                </div>
              )}
              {selectedMember.email && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={`mailto:${selectedMember.email}`} className="text-orange-600 hover:underline">
                    {selectedMember.email}
                  </a>
                </div>
              )}
      ...
      <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setActiveTab('events')} className="text-sm text-orange-600 hover:text-orange-700">
              View all →
            </button>
          </div>
              ...
              <div key={member.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                ...
                <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
                  <button
                    ...
                    className="flex-1 text-sm text-orange-600 hover:text-orange-700 font-medium"
                  >
                    View Profile
                  </button>
                  <button
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setShowEventModal(true)} className="inline-flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700">
            <Plus size={20} />
            New Event
          </button>
        </div>
