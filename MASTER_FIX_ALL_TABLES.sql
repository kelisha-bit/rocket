-- =====================================================
-- MASTER FIX - ALL TABLES
-- =====================================================
-- This script creates ALL tables with proper foreign keys
...
-- =====================================================
-- STEP 2: CREATE MEMBERS TABLE (BASE TABLE)
-- =====================================================
...
CREATE TABLE members (
  ...
  primary_cell_group_id UUID, -- FK will be added later
  primary_ministry_id UUID, -- FK will be added later
...
-- =====================================================
-- STEP 3: CREATE MINISTRIES TABLE
-- =====================================================
...
-- =====================================================
-- STEP 5: ADD FOREIGN KEYS TO MEMBERS TABLE
-- =====================================================

ALTER TABLE members 
  ADD CONSTRAINT fk_members_primary_ministry_id 
  FOREIGN KEY (primary_ministry_id) REFERENCES ministries(id) ON DELETE SET NULL;

ALTER TABLE members 
  ADD CONSTRAINT fk_members_primary_cell_group_id 
  FOREIGN KEY (primary_cell_group_id) REFERENCES cell_groups(id) ON DELETE SET NULL;
...
SELECT '✓ Foreign keys added to members table' as status;

-- =====================================================
-- STEP 6: CREATE GIVING_TRANSACTIONS TABLE
-- =====================================================
...
-- Create indexes for giving_transactions
...
-- Create trigger for giving_transactions
...
-- Enable RLS for giving_transactions
...
SELECT '✓ Giving Transactions table created: ' || COUNT(*) || ' records' as status FROM giving_transactions;
...
UNION ALL
SELECT 'Giving Transactions', COUNT(*) FROM giving_transactions;

-- Verify all foreign keys
SELECT '========== ALL FOREIGN KEYS ==========' as header;
...
SELECT 'Refresh your application now!' as next_step;
SELECT 'All pages should work: members, finance, etc.' as feature;
SELECT '========================================' as footer;